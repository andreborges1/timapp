...
// Weighted type pool: more MEETING/TRAINING than CONFERENCE
const TYPE_POOL = [
...
// Convert Toronto local time to UTC Date
...
async function main() {
  ...
  // ── Generate Events ───────────────────────────────────────────────────────
  // From 35 days ago to 7 days in the future, Mon–Sat (skip Sunday = day 0)
  ...
  while (cursor <= endDate) {
    ...
    if (dayOfWeek !== 0) {
      // Generate 40 events: 3 per hour from 8:00 to 20:50 (hours 8..20)
      // 13 hours × 3 = 39 + 1 extra at a random slot = 40
  ...
  // Only for past events (startTime < now)
  ...
  // Track each user's booked time slots to avoid overlaps
  ...
  for (const event of pastEvents) {
    const eligibleUsers = users.filter((u) => {
      ...
      return !schedule.some(
        (slot) => slot.start < event.endTime && slot.end > event.startTime
      );
    });
    ...
    for (const user of selected) {
      // scannedAt = random time within the event window
